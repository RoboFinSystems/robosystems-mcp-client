Create a GitHub issue for the robosystems-mcp-client repository based on the user's input.
...
2. **Gather Context** - If the user provides a file path or references existing code:
   ...
   - Check tool definitions in `index.js`
   - Review test coverage in `index.test.js`
...
6. **Set Issue Type** - After creation, set the issue type via GraphQL:
   ```bash
   # Get issue ID
   gh api graphql -f query='{ repository(owner: "RoboFinSystems", name: "robosystems-mcp-client") { issue(number: NUMBER) { id } } }'
...
- `area:tools` - MCP tool definitions
- `area:resources` - MCP resource handlers
- `area:api` - RoboSystems API integration
